...
public class BooksBasketManager {
    ...
    public void addToBooksBasket(Book book){
        ...
        System.out.println(book.getTitle() + " by: " + book.getAuthor() + " Added to the basket " + " Price: " + String.format("%.1f", book.getPrice()));
    }
    ...
    public void makePaymentBooks(double payment, double total){
        if (payment < total){
            ...
            clearBooksBasket();
            System.out.println("Payment successful! Your money back: " + String.format("%.1f", (payment - total)));
        } else if (total == 0.0){
